...
export default async function RaceWeekendPage({ searchParams }: { searchParams?: RaceWeekendSearchParams }) {
  ...
  const moduleCards = [
    { title: "比赛结果", code: "CLASSIFICATION", href: moduleHref("/results"), description: "查看完整成绩表、名次、差距、圈数和完赛状态。", meta: winner?.driver ? `当前首位 ${winner.driver}` : "等待成绩数据" },
    { title: "赛控消息", code: "RACE CONTROL", href: moduleHref("/race-control"), description: "查看旗语、安全车、赛道边界、调查和删除圈速等消息。", meta: `${raceControl.data.length} 条消息` },
    { title: "圈速分析", code: "LAP ANALYSIS", href: moduleHref("/lap-analysis"), description: "查看最快圈、分段、位置、差距和 stint 信息。", meta: fastestReference ? `${fastestReference.driver} · ${fastestReference.bestLap}` : "等待圈速数据" },
    { title: "赛道天气", code: "WEATHER", href: moduleHref("/weather"), description: "查看赛道温度、空气温度、湿度、风速和降雨记录。", meta: latestWeather ? `${latestWeather.trackTemperature} · ${latestWeather.rainLabel}` : "等待天气数据" }
  ];
  ...
  const insightSummary = winner
    ...
    : `${selectedSessionName} 当前暂无完整成绩，已展示可用的赛控、圈速和天气入口。`;
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">
              这个页面只保留总览、选择赛段和核心入口。详细结果、赛控、圈速和天气通过导航进入对应页面，不再把所有内容堆成一条长页面。
            </p>
          </div>
