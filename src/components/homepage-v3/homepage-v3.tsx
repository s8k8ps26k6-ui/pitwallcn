...
function getCountdown(targetIso: string, now: Date) {
  ...
  return { days: Math.floor(ms / 86_400_000), hours: Math.floor((ms / 3_600_000) % 24), minutes: Math.floor((ms / 60_000) % 60) };
}
...
function formatShortDate(value: string) {
  return new Intl.DateTimeFormat("zh-CN", { month: "numeric", day: "numeric", timeZone: "UTC" }).format(new Date(`${value}T12:00:00.000Z`));
}

function getGrandPrixTitle(name: string) { return name.replace(/ Grand Prix$/i, ""); }
function getPhaseLabel(phase: "current" | "next" | "off-season") { return phase === "current" ? "比赛周进行中" : phase === "off-season" ? "赛季间歇" : "下一场比赛"; }
...
function getRaceWeekProgress(race: UnifiedRace, now: Date) {
  ...
  if (now.getTime() < start) return { label: "赛前待命", detail: `比赛周始于 ${formatShortDate(race.race.startDate)}`, progress: 0 };
  if (now.getTime() > end) return { label: "比赛周已完成", detail: "等待已核验的赛后结果与复盘。", progress: 100 };
  return { label: "比赛周进行中", detail: "赛段、Live Timing 与赛会信息会按可靠数据源更新。", progress: Math.round(((now.getTime() - start) / (end - start)) * 100) };
}

function getRailState(item: UnifiedRace, selected: UnifiedRace, phase: "current" | "next" | "off-season") {
  if (item.eventId === selected.eventId) return phase === "current" ? "当前" : "下一站";
  return Date.parse(`${item.race.endDate}T23:59:59.999Z`) < Date.now() ? "已结束" : "随后";
}

export function HomepageV3({ race, phase, raceRail }: { race: UnifiedRace; phase: "current" | "next" | "off-season"; raceRail: UnifiedRace[] }) {
  ...
  const style = { "--event-accent": theme.accent, "--event-accent-rgb": theme.accentRgb, "--event-support": theme.support, "--week-progress": `${week.progress}%` } as CSSProperties;
  ...
  useEffect(() => {
    ...
    return () => { if (timer) window.clearInterval(timer); document.removeEventListener("visibilitychange", sync); };
  }, []);
  ...
  return <main className={styles.page} style={style}>
    ...
    <section className={`${styles.hero} ${responsiveStyles.hero}`} aria-labelledby="race-title">
      ...
      <section className={styles.session} aria-label="下一场赛段">
        <p>下一 Session</p><strong>{moment.label}</strong><time dateTime={moment.startTime}>{localSessionTime}</time>
        <div className={styles.countdown} aria-label="倒计时"><span><b>{String(countdown.days).padStart(2, "0")}</b>天</span><span><b>{String(countdown.hours).padStart(2, "0")}</b>时</span><span><b>{String(countdown.minutes).padStart(2, "0")}</b>分</span></div>
        <Link className={styles.primaryAction} href={detailHref}>进入比赛周 <span aria-hidden="true">→</span></Link>
        <div className={styles.secondaryLinks}><Link href="/schedule">查看赛历</Link><Link href="/atlas-v2">探索 Atlas</Link></div>
      </section>
    </section>

    <section className={styles.rail} aria-labelledby="season-rail-title">
      <header><p>赛季轨迹</p><h2 id="season-rail-title">上一站 · 当前站 · 下一站</h2></header>
      <div className={styles.railItems}>{raceRail.map((item) => { const active = item.eventId === race.eventId; return <Link href={`/races/${item.season}/${item.eventId}` as Route} className={`${styles.railItem} ${active ? styles.railItemActive : ""}`} key={item.eventId}><span>{getRailState(item, race, phase)}</span><b>R{String(item.race.round).padStart(2, "0")}</b><i>{getCountryFlag(item.race.country)}</i><strong>{getGrandPrixTitle(item.race.name)}</strong><time>{formatShortDate(item.race.startDate)}</time></Link>; })}</div>
    </section>

    <section className={styles.operations} aria-label="本站运行摘要">
      <div className={styles.weekStatus}><div className={styles.weekRing} aria-hidden="true"><span /></div><div><p>比赛周状态</p><strong>{week.label}</strong><small>{week.detail}</small></div><Link href="/live">Live Timing <em>赛时开启</em><b aria-hidden="true">→</b></Link></div>
