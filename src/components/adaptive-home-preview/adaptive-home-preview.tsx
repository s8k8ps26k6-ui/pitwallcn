...
import { useAdaptiveViewport } from "@/components/adaptive-viewport/use-adaptive-viewport";
import {
  formatLocalDateTime,
  getCountryFlag,
...
import styles from "./adaptive-home-preview.module.css";
...
export function AdaptiveHomePreview({
  race,
  phase,
  raceRail,
}: {
  ...
  const viewport = useAdaptiveViewport();
  ...
  const adaptiveStyle = {
    ...
    "--av-track-scale": viewport.trackScale,
    "--av-vw": viewport.viewportWidth ? `${viewport.viewportWidth}px` : "100vw",
    "--av-vh": viewport.viewportHeight ? `${viewport.viewportHeight}px` : "100dvh",
  } as CSSProperties;
  ...
  const measured = viewport.viewportWidth > 0;
  ...
  return (
    ...
    >
      ...
      <div className={styles.viewportStatus} aria-live="polite">
        <span>ADAPTIVE PREVIEW</span>
        <b>{measured ? `${viewport.viewportWidth}×${viewport.viewportHeight}` : "MEASURING"}</b>
        {measured ? (
          <>
            <em>{viewport.orientation}</em>
            <em>{viewport.layout}</em>
            <small>
              screen {viewport.screenWidth}×{viewport.screenHeight} · DPR {viewport.dpr}
            </small>
          </>
        ) : null}
      </div>

      <section className={styles.hero} aria-labelledby="adaptive-race-title">
      ...
      <section className={styles.rail} aria-labelledby="adaptive-season-rail-title">
        <header>
          <p>赛季轨迹</p>
          <h2 id="adaptive-season-rail-title">上一站 · 当前站 · 下一站</h2>
        </header>
      ...
      <section className={styles.operations} aria-label="本站运行摘要">
        <div className={styles.weekStatus}>
          ...
          <Link href="/live">Live Timing <em>赛时开启</em><b aria-hidden="true">→</b></Link>
        </div>
        <div className={styles.metrics}>
          <p>本站关键数据</p>
          <div>
            <strong>{race.circuit?.lengthKm ? race.circuit.lengthKm.toFixed(3) : "—"}<small>公里</small></strong>
            <strong>{race.circuit?.laps ?? "—"}<small>正赛圈数</small></strong>
            <strong>{race.race.isSprint ? "SPRINT" : "GP"}<small>比赛格式</small></strong>
          </div>
        </div>
      ...
      <footer className={styles.footer}>
        <span>LAPMETRY · ADAPTIVE VIEWPORT LAB</span>
        <span>布局由 visual viewport + container size 驱动</span>
      </footer>
    </main>
  );
}
