...
import { deriveWeekend, matchingMeeting, type WeekendSession } from './weekend-model';
type Meeting = {
    meeting_key: number;
    country_name: string;
    date_start: string;
...
export async function loadWeekend(race: UnifiedRace, nowIso: string, requestedKey?: number) {
    ...
    try {
        ...
        const meeting = matchingMeeting(meetings, race.race.country, race.race.startDate);
        if (meeting) {
            const rows = await fetchOpenF1<ApiSession[]>('/sessions', { meeting_key: meeting.meeting_key });
    ...
    const weatherEntries = await Promise.all(weekend.sessions.map(async (session) => {
        if (!session.sessionKey || Date.parse(session.start) > Date.parse(nowIso))
            return null;
