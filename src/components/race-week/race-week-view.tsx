...
export function RaceWeekView({ race, races, data, invalidEvent }: { race: UnifiedRace; races: UnifiedRace[]; data: WeekendData; invalidEvent?: boolean }) {
  ...
  return <main id="main-content" className={styles.page} data-weekend-state={weekend.state} data-event={race.race.id}>
    ...
    <section className={styles.scene} aria-labelledby="event-title">
      ...
      <div className={styles.overview}><span>RACE WEEK</span><strong>{data.eventDateElapsed && data.scheduleSource === 'calendar' ? '赛周日期已过' : stateLabels[weekend.state]}</strong><p>{focused ? shortSessionName(focused.name) : '赛段记录待补全'}</p></div>
    </section>
      ...
      <div className={styles.days} style={{ '--sessions': Math.max(1, weekend.sessions.length) } as CSSProperties}>
        {[...days].map(([day, indexes]) => <section className={styles.day} key={day} style={{ '--day-sessions': indexes.length } as CSSProperties}>
          ...
          <ol>{indexes.map(i => {
            ...
            return <li key={session.key} data-state={state} data-focus={i === weekend.focus && weekend.state !== 'completed'}>
              <div className={styles.session}><span className={styles.node} aria-hidden="true"/><div><h4>{session.sessionKey ? <Link href={(`/race-weekend?event=${encodeURIComponent(race.eventId)}&session=${session.sessionKey}`) as Route} aria-current={session.key === data.selectedKey ? 'true' : undefined}>{shortSessionName(session.name)}</Link> : shortSessionName(session.name)}</h4><time dateTime={session.confirmed ? session.start : undefined}>{session.confirmed ? local(session.start, zone, { hour: '2-digit', minute: '2-digit', hour12: false }) : '时间待确认'}{session.confirmed && session.end ? ` — ${local(session.end, zone, { hour: '2-digit', minute: '2-digit', hour12: false })}` : ''}</time></div><span className={styles.sessionStatus}>{state === 'live' ? 'LIVE' : state === 'scheduled' ? '按日程进行中' : state === 'completed' ? '已结束' : state === 'elapsed' ? '已到计划时间' : i === weekend.focus && session.confirmed ? '下一场' : !session.confirmed ? data.eventDateElapsed ? '记录待补全' : '待确认' : '待开始'}</span></div>
