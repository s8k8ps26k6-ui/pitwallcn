...
export function deriveWeekend(sessions: WeekendSession[], nowIso: string, liveSessionKey?: number, eventDateElapsed = false) {
    ...
    const states: SessionState[] = ordered.map(session => {
        if (!session.confirmed || !Number.isFinite(Date.parse(session.start)))
            return 'upcoming';
        if (session.end && Date.parse(session.end) <= now)
            return 'completed';
        if (Date.parse(session.start) > now)
            return 'upcoming';
        if (liveSessionKey !== undefined && session.sessionKey === liveSessionKey)
            return 'live';
...
export function matchingMeeting<T extends { country_name: string; date_start: string }>(meetings: T[], country: string, startDate: string): T | undefined {
    const matches = meetings.filter(meeting => meeting.country_name.trim().toLowerCase() === country.trim().toLowerCase() && Math.abs(Date.parse(meeting.date_start) - Date.parse(startDate)) < 3 * 86400000);
